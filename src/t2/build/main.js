...
var _mercadolibre = require('mercadolibre');
...
const app = new _mercadolibre.Meli(process.env.CLIENT_ID, process.env.CLIENT_KEY);
...
const itemToJSON = (type, element) => {
    return {
        ...
        brand: element.attributes && element.attributes[0] && element.attributes[0].value_name ? (0, _removeAccents2.default)(element.attributes[0].value_name) : 'Sem Marca'
    };
};
...
const paging = (query, offset) => new Promise((resolve, reject) => {
    app.get(`sites/MLB/search?q=${query}&offset=${offset}`, (err, res) => {
        if (err) reject(err);else Promise.all(res.results.map(element => {
...
const search = query => new Promise((resolve, reject) => {
    ...
    app.get(`sites/MLB/search?q=${query}`, (err, res) => {
        if (err) reject(err);else {
            ...
            const offset = Array.apply(null, { length: length }).map((element, index) => {
                return (index + 1) * 200;
            });
 ...
 */
_fs2.default.open('results.arff', 'w', (err, fd) => {
...
`;
    if (err) console.log(err);else {
        Promise.all([search('whey'), search('bcaa'), search('termogenico'), search('glutamina'), search('creatina'), search('caixa barra de proteina')]).then(result => {
            return [].concat.apply([], result);
        }).then(result => {
            // Transform elements into string response.
            return result.map(stringfyResponse).join('\n');
        }).then(result => {
            _fs2.default.write(fd, fileHeader.concat(result), (err, written, str) => {
