...
static bool is_friendship(Connection *connection) {
    ...
    if(NULL != connection && NULL != connection_first(connection) && NULL != connection_second(connection)) {
        if(!strcmp(person_name(connection_first(connection)), person_name(connection_second(connection))))
            pontuation += 1;

        if(person_age(connection_first(connection)) == person_age(connection_second(connection)))
            pontuation += 2;

        if(!strcmp(person_course(connection_first(connection)), person_course(connection_second(connection))))
            pontuation += 3;
...
int main(int argc, char **argv) {
    ...
    char *name_one = NULL, *name_two = NULL, *course_one = NULL, *course_two = NULL;
